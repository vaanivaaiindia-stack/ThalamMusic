...
class MusicProvider extends ChangeNotifier {
  ...
  Future<void> _initAudioSession() async {
    ...
    _becomingNoisySub = _session.becomingNoisyEventStream.listen((_) { _audioPlayer.pause(); });
  ...
  Future<void> playSong(int index) async {
    ...
    if (await _session.setActive(true)) {
      try { await _audioPlayer.seek(Duration.zero, index: index); _audioPlayer.play(); }
  ...
  void togglePlayPause() async {
    ...
    else { if (await _session.setActive(true)) { _audioPlayer.play(); } }
    notifyListeners();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          currentScreen,
          if (user.role == UserRole.customer) const Positioned(left: 0, right: 0, bottom: 0, child: PersistentMiniPlayer()),
        ],
      ),
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: const Color(0xFF181820), borderRadius: BorderRadius.circular(20), border: Border.all(color: const Color(0x14FFFFFF)),
          boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.6), blurRadius: 16, offset: const Offset(0, 4))]),
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Row(children: [
            Container(width: 44, height: 44, decoration: BoxDecoration(color: accent, borderRadius: BorderRadius.circular(10)),
              child: Center(child: Text(song.title.substring(0, 2).toUpperCase(), style: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 14)))),
            ...
            Container(width: 36, height: 36, decoration: const BoxDecoration(shape: BoxShape.circle, color: Colors.white),
              child: IconButton(padding: EdgeInsets.zero, icon: Icon(music.player.playing ? Icons.pause : Icons.play_arrow, color: Colors.black, size: 22), onPressed: music.togglePlayPause)),
            IconButton(icon: const Icon(Icons.skip_next, color: Colors.white, size: 22), onPressed: music.next),
          ]),
          const SizedBox(height: 6),
          StreamBuilder<Duration>(stream: music.player.positionStream, builder: (context, snapshot) {
            ...
            return ClipRRect(borderRadius: BorderRadius.circular(2),
              child: LinearProgressIndicator(value: progress, minHeight: 2.5, backgroundColor: Colors.white10, valueColor: AlwaysStoppedAnimation<Color>(accent)));
          }),
        ]),
      ),
    );
...
class FullPlayerScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Padding(padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 12),
        child: Column(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Container(height: 270, width: 270, decoration: BoxDecoration(borderRadius: BorderRadius.circular(32),
            ...
            child: Center(child: Container(width: 110, height: 110, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.black.withOpacity(0.5), border: Border.all(color: accent.withOpacity(0.4), width: 4)),
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            IconButton(icon: Icon(song.isLiked ? Icons.favorite : Icons.favorite_border, color: song.isLiked ? accent : Colors.grey),
              onPressed: () => music.toggleLike(song.id))
          ]),
          StreamBuilder<Duration>(stream: music.player.positionStream, builder: (context, snapshot) {
            final pos = snapshot.data ?? Duration.zero;
            final total = music.player.duration ?? song.duration;
            return Column(children: [
              Slider(value: pos.inSeconds.toDouble().clamp(0.0, total.inSeconds.toDouble()), max: total.inSeconds.toDouble(),
                activeColor: accent, inactiveColor: Colors.white24, onChanged: (val) => music.player.seek(Duration(seconds: val.toInt()))),
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
            ...
            GestureDetector(onTap: music.togglePlayPause,
              child: Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: accent, shape: BoxShape.circle),
...
class _SearchScreenState extends State<SearchScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(padding: const EdgeInsets.only(left: 16, right: 16, top: 12, bottom: 95),
      child: Column(children: [
        TextField(onChanged: (v) => setState(() => query = v),
          decoration: InputDecoration(hintText: 'Search songs, artists, raagas...', prefixIcon: const Icon(Icons.search),
            filled: true, fillColor: const Color(0xFF161622), border: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: BorderSide.none))),
        const SizedBox(height: 16),
        Expanded(child: ListView.builder(itemCount: results.length, itemBuilder: (context, idx) {
          final song = results[idx];
          return ListTile(leading: const Icon(Icons.music_note), title: Text(song.title), subtitle: Text('${song.artist} • ${song.album}'),
...
class LibraryScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return DefaultTabController(length: 3,
      child: Column(children: [
        TabBar(indicatorColor: accent, labelColor: accent, unselectedLabelColor: Colors.grey,
          tabs: const [Tab(text: 'Downloads'), Tab(text: 'Liked Songs'), Tab(text: 'Playlists')]),
        Expanded(child: TabBarView(children: [
          ListView(padding: const EdgeInsets.only(bottom: 95), children: music.playlist.where((s) => s.isDownloaded).map((s) => ListTile(
            leading: const Icon(Icons.download_done, color: Colors.green), title: Text(s.title), subtitle: Text(s.artist),
            onTap: () => music.playSong(music.playlist.indexOf(s)))).toList()),
          ListView(padding: const EdgeInsets.only(bottom: 95), children: music.playlist.where((s) => s.isLiked).map((s) => ListTile(
            leading: const Icon(Icons.favorite, color: Colors.red), title: Text(s.title), subtitle: Text(s.artist),
            onTap: () => music.playSong(music.playlist.indexOf(s)))).toList()),
          ListView(padding: const EdgeInsets.all(16), children: [
            ListTile(tileColor: const Color(0xFF161622), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
              leading: Icon(Icons.add_box, color: accent, size: 36), title: const Text('Create New Playlist'), subtitle: const Text('Curate your custom Spatial sound mix'))
          ]),
        ])),
      ]),
    );
...
class ProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.only(left: 20, right: 20, top: 20, bottom: 95), children: [
      Center(child: CircleAvatar(radius: 44, backgroundColor: accent, child: const Text('TL', style: TextStyle(fontSize: 26, fontWeight: FontWeight.bold, color: Colors.black)))),
      ...
      Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: const Color(0xFF161622), borderRadius: BorderRadius.circular(16), border: Border.all(color: accent.withOpacity(0.3))),
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
      ...
      const SizedBox(height: 24),
      OutlinedButton.icon(style: OutlinedButton.styleFrom(foregroundColor: Colors.red), icon: const Icon(Icons.logout), label: const Text('Sign Out'), onPressed: () => user.logout())
    ]);
  }
}
...
class _VendorScreenState extends State<VendorScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(gradient: LinearGradient(colors: [accent.withOpacity(0.2), const Color(0xFF121218)]), borderRadius: BorderRadius.circular(20), border: Border.all(color: accent.withOpacity(0.5))),
      ...
      const SizedBox(height: 16),
      TextField(controller: _nameCtrl, decoration: InputDecoration(labelText: 'Name', prefixIcon: const Icon(Icons.person), filled: true, fillColor: const Color(0xFF161622), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none))),
      const SizedBox(height: 12),
      ...
      ...context.watch<MusicProvider>().submissions.map((sub) {
        return Card(color: const Color(0xFF14141C), margin: const EdgeInsets.only(bottom: 10), child: ListTile(
          ...
          title: Text(sub.title), subtitle: Text("${sub.artist} • ${sub.genre}"),
          trailing: Text(sub.status, style: TextStyle(color: sub.status == 'Approved' ? Colors.green : (sub.status == 'Rejected' ? Colors.red : Colors.orange), fontWeight: FontWeight.bold)),
        ));
      }).toList(),
    ]);
  }
...
class _AdminLoginScreenState extends State<AdminLoginScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(backgroundColor: const Color(0xFF050505), appBar: AppBar(title: const Text('Admin Login'), backgroundColor: Colors.transparent),
...
class AdminScreen extends StatelessWidget {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      ...
      ...music.submissions.map((sub) {
        return Card(color: const Color(0xFF14141C), margin: const EdgeInsets.only(bottom: 10), child: ListTile(
          leading: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: accent.withOpacity(0.2), borderRadius: BorderRadius.circular(10)), child: Icon(Icons.audio_file, color: accent)),
          title: Text(sub.title), subtitle: Text("${sub.artist} • ${sub.genre}"),
          trailing: DropdownButton<String>(value: sub.status, dropdownColor: const Color(0xFF14141C), style: const TextStyle(color: Colors.white, fontSize: 12), underline: Container(height: 1, color: accent),
            items: const [DropdownMenuItem(value: 'Approved', child: Text('Approved')), DropdownMenuItem(value: 'Rejected', child: Text('Rejected')), DropdownMenuItem(value: 'Pending Review', child: Text('Pending'))],
            onChanged: (val) { if (val != null) { music.updateSubmissionStatus(sub.id, val); } }),
        ));
      }).toList(),
    ]);
  }
  ...
  Widget _buildStatCard(BuildContext context, String title, String value, IconData icon) {
    final accent = context.watch<UserProvider>().goldAccent;
    return Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: const Color(0xFF14141C), borderRadius: BorderRadius.circular(16), border: Border.all(color: const Color(0x14FFFFFF))),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
